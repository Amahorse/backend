...
class Package  {
  ...
  public static function getComponent(array $component, $price = true):array|bool {
    ...
    Store::getInstance()->setQuantity($component['quantity']);
    
    //Non prendo prezzi perchè mi serve solo il prezzo iva esclusa da sommare a quello base del pacchetto
    if($cmp = Store::getInstance()->get($component['id_store_products_component'],$price)) {
  ...
  public static function getValues(array $package):array {
    ...
    //Preset tipo pacchetto con segui regole configuratore prende le regole configuratore del prodotto base per il prezzo iva esclusa
    $recalc = false;
    ...
    foreach($package['components'] as $component) {
        ...
        if($package['rules_configurator_availability'] == 1) {

            //Controlla disponibilità pacchetti
            if($component['availability_warehouse'] == 0 && $component['availability_virtual'] == 0) {
                $package['status'] = 'out_of_stock';
            }

            if($component['status'] == 'not_on_sale') {
                $package['status'] = 'not_on_sale';
            }
