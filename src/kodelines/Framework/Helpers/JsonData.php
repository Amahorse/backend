...
class JsonData {
  ...
  public static function list():array {
    ...
    if(!$folder = Folder::read(uploads().'json')) {
      return [];
    } 
  ...
  public static function get(string $file, $mode = 'array'):mixed {
    ...
    if($mode == 'array') {
      return Json::arrayFromFile(uploads(). 'json/' . $file);
    }

    return file_get_contents(uploads(). 'json/' .$file);
  }
  ...
  public static function update(string $file,$json):bool {
    ...
    return (bool)file_put_contents(uploads() . 'json/'. $file,$json);
